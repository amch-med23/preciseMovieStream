...
import requests
import asyncio
# importing the mail from the email_handler model

from models.email_handler import mail
    ...
    # awaiting dara_check
        ...
        if check_obj['email_check'] == 'passed' and check_obj['password_check'] == 'passed':
            ...
            try:
                ...
                print('before handeling the email')
                
                handle_email(session_id, user_email)# awaiting email_handeling in here.

                print('after handelling the email')
            except Exception as e:
                print('Error ocured while handeling email inside index (for route /register): {}'. format(e))
        ...
        print('errors encountered in index/registe function {}'.format(e))
...
def check_if_email_exist():
    ...
    if status:
        ...
        # in here we gonna send the reset email object returned from the compose reset_email
        try :
            # sending the email
            msg = Message(subject=reset_email_obj['email_subject'], sender='no-replay@losag.tech', recipients=[reset_email_obj['user_email']])
            msg.html = reset_email_obj['email_html']
            mail.send(msg)
            print('The password reset email was sent successfully.')
            #print('this was the content of the email: {}'. format(reset_email_obj['email_html']))
        except Exception as e:
            print('eception occured while sending the reset email: {}'.format(e))
